...
class CustomLogger(LoggerClass):  # type: ignore[valid-type, misc]

    """Custom implementation of the `Logger` class with an added `trace` method."""

    def trace(self, msg: str, *args, **kwargs) -> None:  # type: ignore[no-untyped-def]
        """
...
def _set_trace_loggers() -> None:
    """
    Set loggers to the trace level according to
    the value from the BOT_TRACE_LOGGERS env var.

    When the env var is a list of logger names delimited by a comma,
    each of the listed loggers will be set to the trace level.

    If this list is prefixed with a "!", all of the loggers,
    except the listed ones will be set to the trace level.
